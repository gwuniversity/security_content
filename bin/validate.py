...
def validate_schema(REPO_PATH, type, objects):
    ...
    if type == 'detections':
        manifest_files = glob.glob(path.join(path.expanduser(REPO_PATH), type, '**/*.yml'), recursive=True)
        objects_by_dir = {}

        for manifest_file in manifest_files:
            parent_dir = path.dirname(manifest_file)
            if parent_dir not in objects_by_dir:
                objects_by_dir[parent_dir] = []
            ...
            objects_by_dir[parent_dir].append(object)

            for parent_dir, dir_objects in objects_by_dir.items():
                dir_name = path.basename(parent_dir)
                if type in objects:
                    objects[type].extend(dir_objects)
                else:
                    objects[type] = dir_objects
...
def validate_objects(REPO_PATH, objects):
    ...
    for object_type in ['stories', 'baselines', 'response_tasks', 'responses']:
        objects_array = objects.get(object_type, [])
        errors = errors + validation_errors

    if 'detections' in objects:
        for parent_dir, dir_objects in objects['detections'].items():
            for object in dir_objects:
                validation_errors, uuids = validate_standard_fields(object, uuids)
                errors = errors + validation_errors

    if 'baselines' in objects:
